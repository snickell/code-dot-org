    ...
    module Dashboard
      module CurriculumContent
        class SyncOut < I18n::Utils::SyncOutBase
          # The ScripttmsSerializer used in the sync in organizes all content by
          ...
          def process(language)
            tms_locale_dir = tms_locale_dir_of(language)
          ...
          private def tms_locale_dir_of(language)
            I18nScriptUtils.tms_locale_dir(language[:locale_s], DIR_NAME)
          end

          # We use URLs as keys for lessons in tms, to make things easier for translators.
          ...
          # We use URLs as keys for reference_guides in tms, to make things easier for translators.
          # For the actual translation files, though, we'd like to use more-standard keys.
          ...
          # tmsCollectionSerializer) and a Serializer for the top-level object,
          ...
          # Restores redacted tms file content
          #
          # @param tms_file_path [String] the full tms file path
          # @return [Hash] the restored i18n data or an empty hash
          private def restore_file_content(tms_file_path)
            # Separates the file subpath (e.g. `2017/csd/csd1-2017.json`)
            # from the tms file path (e.g. `i18n/locales/source/curriculum_content/2017/csd/csd1-2017.json`)
            file_subpath = tms_file_path.partition(DIR_NAME).last

            original_file_path = File.join(I18N_BACKUP_DIR_PATH, file_subpath)
            RedactRestoreUtils.restore(original_file_path, tms_file_path, tms_file_path, REDACT_RESTORE_PLUGINS)
            I18nScriptUtils.parse_file(tms_file_path) || {}
          end

          private def types_i18n_data_of(language)
            types_i18n_data = {}

            tms_file_paths = Dir.glob(File.join(tms_locale_dir_of(language), '**/*.json'))
            mutex.synchronize {progress_bar.total += tms_file_paths.size}

            # First we gather together all our script objects into a single hash
            tms_file_paths.each do |tms_file_path|
              type_i18n_data = restore_file_content(tms_file_path)
              type = File.basename(tms_file_path, '.json')
