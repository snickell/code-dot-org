...
export const DefaultRadioButton: StoryObj<typeof RadioButton> = SingleTemplate.bind({});
...
DefaultRadioButton.play = async ({canvasElement}) => {
  // Clicking on the label should mark the radio input as checked
  const canvas = within(canvasElement);

  const radioLabel = canvas.getByText('RadioButton 1');
  await userEvent.click(radioLabel)

  const radio: HTMLInputElement = canvas.getByLabelText('RadioButton 1', {selector: 'input'})

  await expect(radio.checked).toEqual(true);
}
