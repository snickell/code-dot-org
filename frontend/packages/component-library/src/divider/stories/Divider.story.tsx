...
export const GroupOfDividersWithMargin: StoryObj<{
  ...
  play: async ({canvasElement}) => {
    const canvas = within(canvasElement);
    const dividerElements = await canvas.getAllByRole('separator');
    const expectedColor = window
      .getComputedStyle(document.body)
      .getPropertyValue('--background-neutral-quaternary');
    ...
    dividerElements.forEach(dividerElement => {
      ...
      // applies a margin
      expect(dividerElement).not.toHaveStyle('margin: 0px');
    });
  },
};
