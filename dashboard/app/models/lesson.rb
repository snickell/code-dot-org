...
class Lesson < ApplicationRecord
  ...
  def related_csf_lessons
    ...
    lessons = Lesson.eager_load(script: :course_version).
      where("JSON_EXTRACT(scripts.properties, '$.curriculum_umbrella') = ?", script.curriculum_umbrella).
      where(key: key).
      ...
      order(Arel.sql("JSON_EXTRACT(scripts.properties, '$.version_year')"), 'scripts.name')
    lessons - [self]
  end
